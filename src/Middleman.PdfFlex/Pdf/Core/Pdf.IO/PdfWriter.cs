...
namespace Middleman.PdfFlex.Pdf.IO
{
    ...
    class PdfWriter
    {
        ...
        public void WriteStream(PdfDictionary value, bool omitStream)
        {
            ...
            {
                // Earlier versions of PdfFlex skipped the '\n' before 'endstream' if the last byte of
        ...
        {
            // ISO 19005-1:2005 clause 6.1.8: obj must be immediately followed by an EOL marker.
            // Verbose comments go on their own line before the obj line.
            if (Layout == PdfWriterLayout.Verbose)
            {
        ...
        public void WriteFileHeader(PdfDocument document)
        {
            ...
            if (Layout == PdfWriterLayout.Verbose)
            {
                WriteRaw($"% {PdfFlexProductVersionInformation.Title} {PdfFlexProductVersionInformation.Version} (verbose mode)\n");
                ...
                WriteRaw("%                                                \n");  // Objects placeholder
#if DEBUG
                WriteRaw("% This document is created from a DEBUG build. Do not use a DEBUG build for production.\n");
#endif
                WriteRaw("%--------------------------------------------------------------------------------------------------\n");
            }
        }
        ...
        {
            WriteRaw($"% Created with {PdfFlexProductVersionInformation.Title} {PdfFlexProductVersionInformation.SemanticVersion} ({Capabilities.Build.BuildName}) under .NET {Capabilities.Build.Framework}\n");
            WriteRaw("startxref\n");
