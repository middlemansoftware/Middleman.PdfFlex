...
namespace Middleman.PdfFlex.Pdf.IO
{
    ...
    public sealed class PdfReader
    {
        ...
        PdfDocument OpenFromFile(string path, string? password, PdfDocumentOpenMode openMode, PdfPasswordProvider? passwordProvider)
        {
            ...
            {
                PdfFlexLogHost.Logger.LogError(ex, "Open a PDF document failed.");
                throw;
            }
            return document!;
        }
            ...
            PdfPasswordProvider? passwordProvider, PdfReaderOptions? options = null)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (/*sizeof(SizeType) == 4 &&*/ stream.Length > Int32.MaxValue)
                        throw new PdfReaderException(
                            ...
                            "Recompile PdfFlex with USE_LONG_SIZE set in Directory.Build.targets");
#endif
                ...
                else
                {
                    if (password != null)
                    {
                        PdfFlexLogHost.Logger.LogWarning("Password specified but document is not encrypted.");
                        // Ignore the password.
                    }
                }
                ...
                if (openMode == PdfDocumentOpenMode.Modify)
                {
                    ...
                    if (removed != 0)
                    {
                        ...
                        PdfFlexLogHost.PdfReadingLogger.LogInformation("Number of deleted unreachable objects: {Removed}", removed);
                    }
