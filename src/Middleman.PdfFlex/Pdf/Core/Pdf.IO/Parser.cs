    ...
    */
    ...
    sealed class Parser
    {
        ...
        public Parser(PdfDocument document, PdfReaderOptions options, ILogger? logger)
        {
            ...
            _logger = logger ?? PdfFlexLogHost.PdfReadingLogger;
        }
        ...
        public Parser(PdfDocument document, Stream objectStream, Parser documentParser)
        {
            ...
            _logger = documentParser._logger ?? PdfFlexLogHost.PdfReadingLogger;
        }
        ...
        PdfObject ReadObjectInternal(PdfObject? pdfObject, PdfObjectID objectID, bool includeReferences, bool fromObjectStream, SuppressExceptions? suppressObjectOrderExceptions)
        {
            ...
            else
            {
                ...
                if (iref != null)
                {
                    ...
                    if (iref.Value != null!)
                    {
                        PdfFlexLogHost.Logger.LogWarning("Another instance of object {iref} was found. Using previously encountered object instead.", iref);
                    ...
                    if (iref.Position >= 0)
                    {
                        PdfFlexLogHost.Logger.LogWarning("Another instance of object {iref} was found. Keeping reference to previously encountered object.", iref);
            ...
            {
                // Investigate if this happens. Please send us the PDF file for analysing it (issues (at) pdfflex.net).
                PdfFlexLogHost.Logger.LogError("Something happened in an object stream that is not expected but can be ignored.");
            ...
            if (!TryReadEndStream(dict, startPosition, ref streamLength, suppressObjectOrderExceptions))
            {
                ...
                if (retryCount != 0)
                {
                    throw new InvalidOperationException(
                        ...
                        $"If you think this is a bug in PdfFlex, please visit {UrlLiterals.LinkToCannotOpenPdfFile} for further information.");
                }
                retryCount++;
        ...
        int GetStreamLength(PdfDictionary dict, SuppressExceptions? suppressObjectOrderExceptions)
        {
            ...
            if (lengthItem is PdfReference reference)
            {
                ...
                {
                    // If somebody came here, please send us your PDF file so that we can fix it (issues (at) pdfflex.net).
            ...
            PdfFlexLogHost.Logger.LogError("Object '{Object}' has no valid /Length entry. Try to determine stream length by looking for 'endstream'.",
            ...
            PdfFlexLogHost.Logger.LogInformation("Determined stream length of object '{Object}' is {StreamLength}.",
        ...
        bool TryReadEndStream(PdfDictionary dict, SizeType streamStart, ref int streamLength, SuppressExceptions? suppressObjectOrderExceptions)
        {
            ...
            PdfFlexLogHost.Logger.LogError(
            ...
            PdfFlexLogHost.Logger.LogInformation(
        ...
        internal PdfObject ReadIndirectObject(PdfReference pdfReference, SuppressExceptions? suppressObjectOrderExceptions, bool withoutDecrypting = false)
        {
            ...
            catch (Exception ex)
            {
                ...
                PdfFlexLogHost.PdfReadingLogger.LogError(ex.Message);
        ...
        PdfObject ReadIndirectObjectFromObjectStreamInternal(PdfObjectID objectID, SuppressExceptions? suppressObjectOrderExceptions)
        {
            ...
            while (doNextRound)
            {
                ...
                PdfFlexLogHost.PdfReadingLogger.LogWarning($"Reading indirect object with ID {objectID} from ObjectStream will be retried.");
            ...
            PdfFlexLogHost.PdfReadingLogger.LogError($"Reading indirect object with ID {objectID} from ObjectStream finally failed.");
        ...
        internal void ReadAllObjectStreamsAndTheirReferences()
        {
            ...
            do
            {
                ...
                foreach (var objectStreamID in thisObjectStreamIDsToLoad)
                {
                    ...
                    if (SuppressExceptions.HasError(suppressObjectOrderExceptions))
                    {
                        ...
                        PdfFlexLogHost.PdfReadingLogger.LogWarning("Loading ObjectStream with ID {objectStreamID} will be retried.", objectStreamID);
                    ...
                    foreach (var objectIDWithOffset in objectIDsWithOffset)
                    {
                        ...
                        // PdfFlex reads objects from high addresses down to low addresses.
                        ...
                        else
                        {
                            ...
                            PdfFlexLogHost.PdfReadingLogger.LogWarning("Ignoring object with ID {objectID} because an object with that ID was already read.", objectID);
                        }
                    }
                }
            ...
            {
                PdfFlexLogHost.PdfReadingLogger.LogError($"Loading ObjectStreams with IDs {String.Join(", ", nextObjectStreamIDsToLoad)} finally failed.");
                ...
                foreach (var objectStreamID in objectStreamIDsToLoad)
                {
                    if (!_objectStreamsWithParsers.ContainsKey(objectStreamID))
                    {
                        ...
                        if (SuppressExceptions.HasError(suppressObjectOrderExceptions))
                        {
                            ...
                            PdfFlexLogHost.PdfReadingLogger.LogWarning($"Loading ObjectStream with ID {objectStreamID} will be retried.");
            ...
            if (objectStreamIDsToLoad.Any())
            {
                ...
                PdfFlexLogHost.PdfReadingLogger.LogError($"Loading ObjectStreams with IDs {String.Join(", ", objectStreamIDsToLoad)} finally failed.");
        ...
        internal PdfTrailer ReadTrailer()
        {
            ...
            if (idx == -1)
            {
                PdfFlexLogHost.Logger.LogError("Cannot find 'startxref' within the last 1024 bytes of the PDF file.");
                    ...
                    if (symbol == Symbol.Integer)
                    {
                        ...
                        for (int id = start; id < start + length; id++)
                        {
                            ...
                            if (!CheckXRefTableEntry(position, id, generation, out var idChecked, out var generationChecked))
                            {
                                ...
                                PdfFlexLogHost.Logger.LogError(message);
                            }
#endif
        ...
        PdfTrailer ReadXRefStream(PdfCrossReferenceTable xrefTable)
        {
            ...
            if (generation != 0)
            {
                ...
                PdfFlexLogHost.Logger.LogError($"Generation number of object '{number} {generation}' which is cross-reference stream shall not be other than zero.");
            }
            ...
            else
            {
                ...
                {
                    PdfFlexLogHost.PdfReadingLogger.LogError("Object '{ObjectID}' already exists in xref table’s references, referring to position {Position}. The latter one referring to position {Position} is used. " +
                                                              $"This should not occur. If you think this is a bug in PdfFlex, please visit {UrlLiterals.LinkToCannotOpenPdfFile} for further information.", oldIref.ObjectID, oldIref.Position, xrefStart);
