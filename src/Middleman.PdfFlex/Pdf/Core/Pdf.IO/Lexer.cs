    ...
    class Lexer
    {
        ...
        TryAgain:
            ...
            switch (ch)
            {
                ...
                default:
                    Debug.Assert(!Char.IsLetter(ch), "PdfFlex did something wrong. See code below.");
        ...
        public Symbol ScanName()
        {
            ...
            while (true)
            {
                ...
                if (ch == '#')
                {
                    ...
                    {
                        PdfFlexLogHost.Logger.LogError("Illegal character {char} in hex string.", ch);
                        return '\0';
                    }
                }
            }
        ...
        internal Symbol ScanNumber(bool testForObjectReference)
        {
            ...
            if (ch is '+' or '-')
            {
                ...
                {
                    PdfFlexLogHost.Logger.LogError("+/- not followed by a number or decimal point.");
                }
            }
...
#pragma warning disable CA2254 // This is a rare case outside xref. Therefor we use string interpolation.
                PdfFlexLogHost.PdfReadingLogger.LogWarning($"Token '{_token}' has more than one leading zero.");
#pragma warning restore CA2254
                            ...
                            switch (ch)
                            {
                                ...
                                default:
                                    ...
                                    else
                                    {
                                        ...
                                        // ParserDiagnostics.HandleUnexpectedCharacter(ch);
                                        PdfFlexLogHost.PdfReadingLogger.LogWarning($"Illegal escape sequence '\\{ch}' found. Reverse solidus (backslash) is ignored.");
                                    }
                                    break;
                            }
                            break;
        ...
        static bool ReadWholeStreamSequence(Stream stream, int length, out byte[] content, out int bytesRead)
        {
            ...
            {
                if (round > 1)
                    PdfFlexLogHost.Logger.StreamIssue(status, bytesRead, length);
            }
        }
