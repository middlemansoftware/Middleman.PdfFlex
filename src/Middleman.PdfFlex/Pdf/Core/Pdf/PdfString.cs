...
namespace Middleman.PdfFlex.Pdf
{
    ...
    enum PdfStringFlags
    {
        ...
        StandardEncoding = 0x01,   // Not used by PdfFlex.
        PDFDocEncoding = 0x02,
        WinAnsiEncoding = 0x03,
        MacRomanEncoding = 0x04,   // Not used by PdfFlex.
        MacExpertEncoding = 0x05,  // Not used by PdfFlex.
        Unicode = 0x06,
        EncodingMask = 0x0F,
        ...
        static bool TryRereadAsUnicode(ref string? value)
        {
            ...
            if (value is ['\xFE', '\xFF', ..])
            {
                ...
                if ((length & 1) == 1)
                {
                    ...
                    // We add a (char)0.
                    PdfFlexLogHost.Logger.LogWarning("A PDF UTF-16 string contains an odd number of bytes. We add a dummy byte of 0.");
