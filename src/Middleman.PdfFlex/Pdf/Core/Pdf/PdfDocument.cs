...
using Middleman.PdfFlex.Events;
...
using Middleman.PdfFlex.Pdf.Security;
using Middleman.PdfFlex.Pdf.Signatures;
...
namespace Middleman.PdfFlex.Pdf
{
    ...
    public sealed class PdfDocument : PdfObject, IDisposable
    {
        ...
        public PdfDocument()
        {
            PdfFlexLogHost.Logger.PdfDocumentCreated(Name);
        ...
        /// The tag is not used by PdfFlex.
        /// </summary>
        public object? Tag { get; set; }

        /// <summary>
        /// Gets or sets the conformance profile for this document (e.g. PDF/A-1b, PDF/UA-1,
        /// or a combination). Setting a conformance profile adjusts the minimum PDF version
        /// and initializes required subsystems such as the UAManager for tagged structure.
        /// </summary>
        public PdfConformance Conformance
        {
            ...
            set
            {
                ...
                if (!value.IsNone)
                {
                    // Raise version if required. SetRequiredVersion only raises, never lowers.
                    SetRequiredVersion(value.MinimumPdfVersion);

                    // Initialize UAManager for conformance profiles that require tagged structure.
                    if (value.RequiresTaggedStructure)
                        _ = UAManager.ForDocument(this);
                }
            }
        }
        PdfConformance _conformance = PdfConformance.None;
        ...
        /// The name is not used by PdfFlex.
        /// </summary>
        ...
        public void Close()
        {
            if (OutStream != null)
            {
                ...
                if (!Conformance.IsNone)
                {
                    PrepareForConformance();
                    ValidateConformance();
                }
        ...
        public async Task SaveAsync(Stream stream, bool closeStream = false)
        {
            ...
            // Prepare and validate conformance (PDF/A, PDF/UA) before saving.
            if (!Conformance.IsNone)
            {
                PrepareForConformance();
                ValidateConformance();
            ...
            if (!CanSave(ref message))
                throw new PdfFlexException(message);
        ...
        {
            PdfFlexLogHost.Logger.PdfDocumentSaved(Name);
        ...
        /// <summary>
        /// Prepares the document for the declared conformance profile. Called at save time
        /// before validation. Sets MarkInfo, output intent, document language, and
        /// DisplayDocTitle as required by the active conformance.
        /// </summary>
        void PrepareForConformance()
        {
            var internals = Internals;
            var conformance = Conformance;

            // ── MarkInfo ──────────────────────────────────────────────
            // UAManager sets MarkInformation when present. If no UAManager,
            // set it directly (needed for PDF/A-1b which doesn't require tagging
            // but does require the Marked flag).
            if (_uaManager == null)
            {
            ...
            // ── Output Intent (ICC color profile) ─────────────────────
            if (conformance.RequiresOutputIntent)
            {
                ...
                outputIntents.Elements.Add("/S", new PdfName(conformance.OutputIntentSubtype!));
                ...
                var profileStream = Assembly.GetExecutingAssembly()
                    .GetManifestResourceStream("Middleman.PdfFlex.Resources.sRGB2014.icc")
                    ?? throw new InvalidOperationException("PdfFlex: Embedded sRGB ICC color profile was not found.");
                ...
                if (read != profileStream.Length)
                    throw new InvalidOperationException("PdfFlex: Embedded sRGB ICC color profile was not fully read.");
            ...
            // ── Document Language ─────────────────────────────────────
            if (conformance.RequiresDocumentLanguage && string.IsNullOrEmpty(Language))
            {
                // Default to English if no language was set by the user.
                Language = "en";
            }

            // ── DisplayDocTitle ───────────────────────────────────────
            if (conformance.RequiresDisplayDocTitle)
            {
        ...
        /// <summary>
        /// Validates that the document meets the requirements of its declared conformance
        /// profile. Called at save time after preparation. Throws <see cref="PdfFlexException"/>
        /// if a required condition is not met.
        /// </summary>
        void ValidateConformance()
        {
            var conformance = Conformance;
            ...
            {
                var structTreeRoot = Catalog.Elements.GetObject(PdfCatalog.Keys.StructTreeRoot);
                if (structTreeRoot == null)
                {
                    throw new PdfFlexException(
                        $"PdfFlex: {conformance} requires a tagged structure tree (StructTreeRoot), " +
                        "but none was found. Ensure the document is built with accessibility tagging enabled.");
                }
            }

            // ── Document Language ─────────────────────────────────────
            if (conformance.RequiresDocumentLanguage)
            {
                if (string.IsNullOrEmpty(Language))
                {
                    throw new PdfFlexException(
                        $"PdfFlex: {conformance} requires a document language (/Lang entry in catalog). " +
                        "Set PdfDocument.Language before saving.");
                }
            }
            ...
            if (conformance.RequiresOutputIntent)
            {
                var outputIntents = Catalog.Elements.GetObject(PdfCatalog.Keys.OutputIntents);
                if (outputIntents == null)
                {
                    throw new PdfFlexException(
                        $"PdfFlex: {conformance} requires an output intent (ICC color profile), " +
                        "but none was found. This is an internal error; please report it.");
                }
            }
        }
        ...
        internal override void PrepareForSave()
        {
            ...
            if (info.Elements[PdfDocumentInformation.Keys.Creator] is null)
                info.Creator = PdfFlexProductVersionInformation.Producer;

            // We set Producer if it is not yet set.
            var pdfProducer = PdfFlexProductVersionInformation.Creator;
#if DEBUG
            ...
            {
                // Prevent endless concatenation if file is edited with PdfFlex more than once.
                if (!producer.StartsWith(PdfFlexProductVersionInformation.Title, StringComparison.Ordinal))
            ...
            if (removed != 0 && PdfFlexLogHost.Logger.IsEnabled(LogLevel.Information))
            {
                PdfFlexLogHost.Logger.LogInformation($"PrepareForSave: Number of deleted unreachable objects: {removed}");
            }
            IrefTable.Renumber();
#endif
        ...
        /// By default, PdfFlex only embeds glyphs of a font that are used for drawing text
