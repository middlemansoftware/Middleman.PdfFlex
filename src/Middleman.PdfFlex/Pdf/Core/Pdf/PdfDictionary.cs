...
namespace Middleman.PdfFlex.Pdf
{
    ...
    public class PdfDictionary : PdfObject, IEnumerable<KeyValuePair<string, PdfItem?>>
    {
        ...
        public sealed class DictionaryElements : IDictionary<string, PdfItem?>, ICloneable
        {
            ...
            public PdfItem? GetValue(string key, VCF options)
            {
                ...
                if (value is null or PdfNull or PdfReference { Value: PdfNullObject })
                {
                    if (options != VCF.None)
                    {
                        ...
                        else
                        {
                            if (key == "/Info")
                            {
                                // We come here if PdfFlex was fully trimmed and meta-data could not be found by reflection.
                                // Note: Should not occur since we added attributes that prevent trimming of certain parts.
                                throw new InvalidOperationException($"PdfFlex relies on reflection and does not work when a fully-trimmed self-contained file is used.\r\n" +
                ...
                else
                {
                    ...
                    if ((iref = value as PdfReference) != null)
                    {
                        ...
                        {
                            var type = GetValueType(key);
                            Debug.Assert(type != null, "No value type specified in meta information. Please send this file to PdfFlex support.");

                            if (type != null! && type != value.GetType())
                            {
                                if (typeof(PdfDictionary).IsAssignableFrom(type))
                                {
                                    Debug.Assert(value is PdfDictionary, "Bug in PdfFlex. Please send this file to PdfFlex support.");
                                    value = CreateDictionary(type, (PdfDictionary)value);
                                }
                                else if (typeof(PdfArray).IsAssignableFrom(type))
                                {
                                    Debug.Assert(value is PdfArray, "Bug in PdfFlex. Please send this file to PdfFlex support.");
            ...
            /// </summary>
            public ArrayOrSingleItemHelper ArrayOrSingleItem => new(this); // TODO_OLD PdfFlex6: Naming.
        ...
        public sealed class PdfStream
        {
            ...
            /// <summary>
            /// Tries to unfilter the bytes of the stream. If the stream is filtered and PdfFlex knows the filter
            ...
            public bool TryUnfilter()
            {
                ...
                if (_value != null)
                {
                    ...
                    if (filter != null)
                    {
                        // PdfFlex can only uncompress streams that are compressed with the ZIP or LZH algorithm.
                        var decodeParms = _ownerDictionary.Elements[Keys.DecodeParms];
