...
namespace Middleman.PdfFlex.Pdf
{
    ...
    public sealed class PdfPage : PdfDictionary, IContentStream
    {
        ...
        /// <summary>
        /// Sets the page dimensions from a <see cref="PageSize"/> struct.
        /// Use one of the standard sizes (e.g. <c>PageSize.A4</c>, <c>PageSize.Letter</c>)
        /// or create a custom size with <c>new PageSize(width, height)</c>.
        /// </summary>
        public PageSize Size
        {
            ...
            set => MediaBox = new(0, 0, value.Width, value.Height);
        }
        ...
        /// </summary>
        // #PageOrientation
        // PdfFlex does not set this value, but for imported pages this value can be set and must be taken
        ...
        {
            // Suppress transparency group if the conformance profile prohibits transparency.
            if (_document.Conformance.AllowsTransparency)
            {
                // Add transparency group to prevent rendering problems of Adobe viewer.
                // Update (PdfFlex 1.50 beta 3): Add transparency group only if ColorMode is defined.
