...
namespace Middleman.PdfFlex.Pdf.Advanced
{
    ...
    public sealed partial class PdfImage : PdfXObject
    {
        ...
        void InitializeJpeg(PdfDocumentOptions options)
        {
            ...
            // Conformance profiles that prohibit image interpolation suppress it.
            if (_image.Interpolate && _document.Conformance.AllowsImageInterpolation)
                Elements[Keys.Interpolate] = PdfBoolean.True;
...
//#endif
        ...
        void CreateIndexedMemoryBitmap(int bits, PdfDocumentOptions options)
        {
...

            {
                ...
                {
                    if (!_document.Conformance.AllowsAlphaMasks)
                    {
                        PdfFlexLogHost.Logger.LogWarning("{Conformance}: Alpha mask of PdfImage suppressed.", _document.Conformance);
                    }
                    else
                    {
                        // The image provides an alpha mask (requires Arcrobat 5.0 or higher).
                ...
                if (_image.Interpolate)
                {
                    if (!_document.Conformance.AllowsImageInterpolation)
                    {
                        PdfFlexLogHost.Logger.LogWarning("{Conformance}: Image interpolation suppressed.", _document.Conformance);
                    }
                    else
                    {
                        Elements[Keys.Interpolate] = PdfBoolean.True;
