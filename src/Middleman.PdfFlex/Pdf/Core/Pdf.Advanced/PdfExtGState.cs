...
namespace Middleman.PdfFlex.Pdf.Advanced
{
    ...
    public sealed class PdfExtGState : PdfDictionary
    {
        ...
        public double StrokeAlpha
        {
            set
            {
                // Conformance profiles that prohibit transparency force alpha to 1.0.
                // ReSharper disable once CompareOfFloatsByEqualityOperator
                if (!_document.Conformance.AllowsTransparency && value != 1.0)
                {
                    PdfFlexLogHost.Logger.LogWarning("{Conformance}: Stroke alpha value set to 1.", _document.Conformance);
                    value = 1.0;
                }
                _strokeAlpha = value;
        ...
        public double NonStrokeAlpha
        {
            set
            {
                ...
                if (!_document.Conformance.AllowsTransparency && value != 1.0)
                {
                    PdfFlexLogHost.Logger.LogWarning("{Conformance}: Non-stroke alpha value set to 1.", _document.Conformance);
                    value = 1.0;
                }
