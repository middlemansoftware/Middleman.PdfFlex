    ...
    sealed class PdfCrossReferenceTable(PdfDocument document) // Must not be derived from PdfObject.
    {
        ...
        public void Add(PdfReference iref)
        {
            if (iref.ObjectID.IsEmpty)
            {
                ...
                PdfFlexLogHost.DocumentProcessingLogger.LogWarning("iRef with empty object ID found.");
            }
            ...
            if (_objectTable.ContainsKey(iref.ObjectID))
            {
                ...
                PdfFlexLogHost.PdfReadingLogger.LogError("Object '{ObjectID}' already exists in xref table’s objects, referring to position {Position}. The latter one referring to position {Position} is used. " +
                                                          $"This should not occur. If you think this is a bug in PdfFlex, please visit {UrlLiterals.LinkToCannotOpenPdfFile} for further information.", oldIref.ObjectID, oldIref.Position, iref.Position);
        ...
        public void Add(PdfObject value)
        {
            ...
            if (value.Owner == null!)
            {
                PdfFlexLogHost.PdfReadingLogger.LogWarning("Object without owner gets owned by the document it was added to.");
            ...
            else
            {
                ...
                {
                    PdfFlexLogHost.PdfReadingLogger.LogError("Object not owned by the document it was added to.");
                }
            }
        ...
        {
            var logger = PdfFlexLogHost.DocumentProcessingLogger;
                ...
                foreach (PdfItem item in enumerable)
                {
                    if (item is PdfReference iref)
                    {
                        ...
                        if (!ReferenceEquals(iref.Document, document))
                        {
                            ...
                            PdfFlexLogHost.PdfReadingLogger.LogError($"Bad iref: {iref.ObjectID.ToString()}");
                        }
