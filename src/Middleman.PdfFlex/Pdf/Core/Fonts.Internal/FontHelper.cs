...
namespace Middleman.PdfFlex.Fonts.Internal
{
    /// <summary>
            ...
            if (descriptor != null)
            {
                ...
                for (int idx = 0; idx < length; idx++)
                {
                    ...
                    if (Char.IsLowSurrogate(ch))
                    {
                        ...
                        PdfFlexLogHost.FontManagementLogger.LogWarning("Unexpected low surrogate found: 0x{Char:X2}", ch);
                        continue;
                    }
                    ...
                    else if (Char.IsHighSurrogate(ch))
                    {
                        ...
                        if (++idx < length)
                        {
                            ...
                            {
                                PdfFlexLogHost.FontManagementLogger.LogWarning("High surrogate 0x{Char:X2} not followed by low surrogate.", ch);
                                continue;
                            }
                        ...
                        {
                            PdfFlexLogHost.FontManagementLogger.LogWarning("High surrogate 0x{Char:X2} found at end of string.", ch);
                            continue;
                        }
                    }
                    else
