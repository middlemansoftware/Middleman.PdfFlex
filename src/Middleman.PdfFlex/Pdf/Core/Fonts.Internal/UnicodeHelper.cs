            ...
            for (int idx = 0; idx < length; idx++)
            {
                ...
                if ((uint)high10Bits <= HighSurrogateRange)
                {
                    ...
                    if ((uint)idx < (uint)s.Length)
                    {
                        ...
                        else
                        {
                            ...
                            PdfFlexLogHost.FontManagementLogger.LogDebug("High surrogate 0x{Char:X2} not followed by a low surrogate.", ch);
                            idx--;
                            continue;
                        }
                    }
                    else
                    {
                        ...
                        PdfFlexLogHost.FontManagementLogger.LogDebug("High surrogate 0x{Char:X2} found at end of string.", ch);
                        break;
                    }
                }
                else
                {
                    ...
                    if ((uint)ch - LowSurrogateStart <= HighSurrogateRange)
                    {
                        ...
                        PdfFlexLogHost.FontManagementLogger.LogDebug("Unexpected low surrogate found: 0x{Char:X2}", ch);
                        continue;
                    }
