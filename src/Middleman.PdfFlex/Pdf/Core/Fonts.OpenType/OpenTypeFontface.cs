    ...
    sealed class OpenTypeFontFace  // Note: In English, it’s spelled 'typeface', but 'font face'.
    {
        ...
        public void SetFontEmbedding(PdfFontEmbedding fontEmbedding)
        {
            ...
            if (fontEmbedding == PdfFontEmbedding.TryComputeSubset)
            {
                ...
                PdfFlexLogHost.Logger.LogError("Font embedding option was already set to EmbedCompleteFontFile. Setting to TryComputeSubset is ignored.");
            }
            else
            {
                // Case: _fontEmbedding is already set to TryComputeSubset.
                PdfFlexLogHost.Logger.LogError("Font embedding option was already set to TryComputeSubset. Now it is reset to EmbedCompleteFontFile.");
                _fontEmbedding = fontEmbedding;
            }
        }
        ...
        internal void Read()
        {
            ...
            try
            {
                ...
                if (startTag == TTCF)
                {
                    ...
                    throw new InvalidOperationException("TrueType collection fonts are not yet supported by PdfFlex.");
                }
                ...
                //    throw new NotSupportedException("Bitmap fonts are not supported by PdfFlex.");
            ...
            {
                PdfFlexLogHost.FontManagementLogger.LogCritical($"Error while reading OpenType font face: {ex.Message}");
                throw;
            }
        }
