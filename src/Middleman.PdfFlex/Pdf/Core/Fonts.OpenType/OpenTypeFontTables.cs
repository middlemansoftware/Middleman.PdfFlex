...
namespace Middleman.PdfFlex.Fonts.OpenType
{
    ...
    /// <summary>
    /// Only Symbol and Unicode are used by PdfFlex.
    /// </summary>
    enum WinEncodingId
    {
        Symbol = 0,
    ...
    class CMapTable : OpenTypeFontTable
    {
        ...
        internal void Read()
        {
            try
            {
                ...
                if (!success)
                    throw new InvalidOperationException("Font has no usable platform or encoding ID. It cannot be used with PdfFlex.");
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException(PsMsgs.ErrorReadingFontData, ex);
