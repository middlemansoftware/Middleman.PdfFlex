...
namespace Middleman.PdfFlex.Fonts.OpenType
{
    ...
    /// Currently, the only font type PdfFlex supports.
    /// </summary>
    sealed class OpenTypeDescriptor : FontDescriptor
    {
        ...
        public ushort CodepointToGlyphIndex(int codePoint)
        {
            if (codePoint <= 0xFFFF)
            {
                PdfFlexLogHost.FontManagementLogger.LogWarning("For code points from the BMP call BmpCharacterToGlyphID directly.");
        ...
        public int GlyphIndexToPdfWidth(ushort glyphIndex)
        {
            ...
            {
                PdfFlexLogHost.FontManagementLogger.LogError("Invalid glyph index hmtx: 0x{Glyph:X4}", glyphIndex);
                throw;
            }
        }
        ...
        public double GlyphIndexToEmWidth(uint glyphIndex, double emSize)
        {
            ...
            {
                PdfFlexLogHost.Logger.LogError(ex, "Error calculating em-size for glyph 0x{Glyph:X4}.", glyphIndex);
                throw;
            }
        }
        ...
        public int GlyphIndexToWidth(int glyphIndex)
        {
            ...
            {
                PdfFlexLogHost.Logger.LogError(ex, "Error find advance width for glyph 0x{Glyph:X4}.", glyphIndex);
                throw;
            }
        }
            ...
            if (IsSymbolFont)
            {
                for (int idx = 0; idx < count; idx++)
                {
                    ...
                    if ((ch & 0xFF00) != 0)
                    {
                        ...
                        PdfFlexLogHost.FontManagementLogger.LogDebug("Unexpected character found for symbol font: 0x{Char:X2}", ch);
                    }
        ...
        public char RemapSymbolChar(char ch)
        {
            ...
            if (ch > 255 && !((uint)(ch - 0xf000) <= 0xff))
            {
                ...
                PdfFlexLogHost.FontManagementLogger.LogError("Character 0u{char} of a symbol font is not in valid range.", value);
            }

            // Used | instead of + because of: http://pdfflex.codeplex.com/workitem/15954
